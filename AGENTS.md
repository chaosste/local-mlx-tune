...
- Apple M1 Mac (8 GB unified RAM); Python 3.12 venv at `~/Projects/local-mlx-tune/.venv`; MLX LoRA via mlx-lm + mlx-tune (unsloth-buddy optional).
...
- Mac2 dedicated to MLX training (Terminal only, minimal apps); Mac1 for Cursor/dev; sync via git and rsync base model once; no Cursor on Mac2.
